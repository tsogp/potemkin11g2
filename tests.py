...
from modified_query import DB

class TestDB(unittest.TestCase):
    def setUp(self):
        self.__tempdb = DB(":memory:")
        self.__tempdb.getCursor.executescript(
            """
            BEGIN TRANSACTION;
    ...
    def tearDown(self):
        self.__tempdb.getSession.close()

    def test_getProfileInfo(self):
        request = self.__tempdb.getProfileInfo(user_id=1)
        self.assertEqual(len(request), 1)

        request = self.__tempdb.getProfileInfo(user_id=2)
        self.assertEqual(len(request), 1)

    def test_getCartInfo(self):
        request = self.__tempdb.getCartInfo(user_id=1)
        self.assertEqual(len(request), 1)

        request = self.__tempdb.getCartInfo(user_id=2)
        self.assertEqual(len(request), 0)
    def test_getPurchasedOrders(self):
        request = self.__tempdb.getPurchasedOrders(user_id=1)
        self.assertEqual(len(request), 1)

        request = self.__tempdb.getPurchasedOrders(user_id=2)
        self.assertEqual(len(request), 1)
